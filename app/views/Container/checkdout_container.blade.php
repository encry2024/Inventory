...
$(document).ready( function() {
	...
	$.getJSON("{{ URL::to('/') }}/getChkOut/{{ $category->id }}",function(data) {
		$('#checkedout').dataTable({
			"aaData": data,
			"aaSorting": [[ 3, 'desc' ]],
			"oLanguage": {
			...
			[
				...
				},
				{
					...
					if ( full["location_id"] != "0" ) {
						return '<a href="{{ URL::to("/") }}/location/' + full["location_id"] + '/profile" class="text-center radius size-14 lrv-link"> ' + full["owner_name"] + '</a>';
					} else if ( full["status"] != "Normal" ) {
						return '<label class="text-center size-14 radius alert-box alert alert-custom">Not Available</label>';
					} else if ( full["status"] == "Normal" ) {
						return '<label class="text-center size-14 radius alert-box success alert-custom">Available</label>';
					}
				}},
				//dateofapplication
				{
